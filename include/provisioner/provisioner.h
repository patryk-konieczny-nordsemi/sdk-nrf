...
*/
#define PROVISIONER_RRAM_ERASE_ALIGN 16U

/** @brief Maximum number of RRAM bytes the provisioner can erase in one entry. */
#define PROVISIONER_RRAM_ERASE_MAX_LEN 64U
...
struct provisioner_generic_callback {
...
#define PROVISIONER_SECRET_STORAGE_ASSERT(_data, _payload_length, _storage_length)                    \
    BUILD_ASSERT((_storage_length) > 0U, "storage_length must be non-zero for secret entries"); \
    BUILD_ASSERT((_storage_length) <= PROVISIONER_RRAM_ERASE_MAX_LEN,                             \
    ...
    BUILD_ASSERT(((_storage_length) % PROVISIONER_RRAM_ERASE_ALIGN) == 0U,                        \
            "storage_length must be RRAM write-block aligned");                            \
    BUILD_ASSERT((((uintptr_t)(_data)) % PROVISIONER_RRAM_ERASE_ALIGN) == 0U,                    \
            "secret data address must be RRAM write-block aligned")

/** @brief Register a generic post-provisioning callback. */
#define PROVISIONER_ENTRY_GENERIC_CALLBACK_REGISTER(_name, _callback)                                   \
    static const STRUCT_SECTION_ITERABLE(provisioner_generic_callback, _name) = {               \
        .name = STRINGIFY(_name),                                                           \
        .callback = (_callback),                                                            \
    }

/** @brief Register a PSA ITS entry (source data retained in the image). */
#define PROVISIONER_ENTRY_ITS_REGISTER(_name, _data, _payload_length, _format, _uid,              \
                    _create_flags)                                               \
...
*/
#define PROVISIONER_ENTRY_ITS_REGISTER_SECRET(_name, _data, _payload_length, _storage_length,     \
                        _format, _uid, _create_flags)                       \
    PROVISIONER_SECRET_STORAGE_ASSERT(_data, _payload_length, _storage_length);                 \
    static const STRUCT_SECTION_ITERABLE(provisioner_its_entry, _name) = {                      \
        .name = STRINGIFY(_name),                                                           \
        .data = (_data),                                                                    \
        .payload_length = (_payload_length),                                                \
...
#define PROVISIONER_ENTRY_KMU_REGISTER(_name, _data, _payload_length, _key_bits, _format, _id,    \
...
*                        >= @p _payload_length, typically `sizeof(_data)`).
*/
#define PROVISIONER_ENTRY_KMU_REGISTER_SECRET(_name, _data, _payload_length, _storage_length,    \
