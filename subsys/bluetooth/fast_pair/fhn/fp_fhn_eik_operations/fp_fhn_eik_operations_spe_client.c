...
int fp_fhn_eik_delete(void)
{
	...
	struct fp_fhn_eik_secure_req req = {
		.op = FP_FHN_EIK_SECURE_OP_DELETE,
	};
...
int fp_fhn_eik_is_provisioned(void)
{
	...
	struct fp_fhn_eik_secure_req req = {
		.op = FP_FHN_EIK_SECURE_OP_IS_PROVISIONED,
	};
...
bool fp_fhn_eik_hash_compare(const uint8_t *eik_hash, const uint8_t *random_nonce)
{
	...
	struct fp_fhn_eik_secure_req req = {
		.op = FP_FHN_EIK_SECURE_OP_HASH_COMPARE,
	};

	psa_invec in_vec[] = {
		{ &req, sizeof(req) },
		{ eik_hash, FP_FHN_EIK_COMMON_HASH_COMPARE_LEN },
		{ random_nonce, FP_FHN_EIK_COMMON_RANDOM_NONCE_LEN },
	};
			  ...
			  uint8_t *fhn_frame_hashed_flags_xor_operand)
{
	...
	struct fp_fhn_eik_secure_req req = {
		.op = FP_FHN_EIK_SECURE_OP_EID_ENCODE,
	};
...
int fp_fhn_eik_provision_encrypted(const uint8_t *encrypted_eik, const uint8_t *account_key)
{
	...
	struct fp_fhn_eik_secure_req req = {
		.op = FP_FHN_EIK_SECURE_OP_PROVISION_ENCRYPTED,
	};
...
int fp_fhn_eik_get_encrypted(const uint8_t *owner_account_key, uint8_t *encrypted_eik)
{
	...
	struct fp_fhn_eik_secure_req req = {
		.op = FP_FHN_EIK_SECURE_OP_GET_ENCRYPTED,
	};
			  ...
			  size_t eik_derived_key_len)
{
	...
	struct fp_fhn_eik_secure_req req = {
		.op = FP_FHN_EIK_SECURE_OP_DERIVE_KEY,
	};
