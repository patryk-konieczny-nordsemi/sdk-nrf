				    ...
				    const uint8_t *random_nonce)
{
	...
	uint8_t hash_input[FP_FHN_STATE_EIK_LEN + FP_FHN_EIK_COMMON_RANDOM_NONCE_LEN];
	...
	memcpy(hash_input + FP_FHN_STATE_EIK_LEN, random_nonce,
	       FP_FHN_EIK_COMMON_RANDOM_NONCE_LEN);
	...
	return !memcmp(eik_hash_local, eik_hash, FP_FHN_EIK_COMMON_HASH_COMPARE_LEN);
}
...
int fp_fhn_eik_common_decrypt(const uint8_t *encrypted_eik, const uint8_t *account_key,
			      uint8_t *eik)
{
	int err;
...
int fp_fhn_eik_common_encrypt(const uint8_t *eik, const uint8_t *owner_account_key,
				 ...
				 uint8_t *eik_derived_key, size_t eik_derived_key_len)
{
	...
	__ASSERT_NO_MSG(eik_derived_key_len <= FP_FHN_EIK_COMMON_DERIVED_KEY_MAX_LEN);
