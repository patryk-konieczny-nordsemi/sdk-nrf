...
#include "fp_fhn_eik_secure_ipc.h"
...
static psa_status_t tfm_eik_hash_compare(const psa_msg_t *msg)
{
	...
	uint8_t eik_hash[FP_FHN_EIK_COMMON_HASH_COMPARE_LEN];
	uint8_t random_nonce[FP_FHN_EIK_COMMON_RANDOM_NONCE_LEN];
	psa_status_t status;
	bool result;
	...
	result = fp_fhn_eik_common_hash_compare(eik, eik_hash, random_nonce);
...
static psa_status_t tfm_eik_eid_encode(const psa_msg_t *msg)
{
	...
	result = fp_fhn_eik_common_eid_encode(eik, eid_seed_buf_data, fhn_eid,
...
static psa_status_t tfm_eik_provision_encrypted(const psa_msg_t *msg)
{
	...
	result = fp_fhn_eik_common_provision_encrypted(encrypted_eik, account_key, eik);
	if (result != 0) {
		mbedtls_platform_zeroize(eik, sizeof(eik));
		return PSA_ERROR_GENERIC_ERROR;
	}
...
static psa_status_t tfm_eik_get_encrypted(const psa_msg_t *msg)
{
	...
	result = fp_fhn_eik_common_get_encrypted(eik, owner_account_key, encrypted_eik);
	mbedtls_platform_zeroize(eik, sizeof(eik));
	if (result != 0) {
		return PSA_ERROR_GENERIC_ERROR;
	}
...
static psa_status_t tfm_eik_derive_key(const psa_msg_t *msg)
{
	...
	uint8_t eik_derived_key[FP_FHN_EIK_COMMON_DERIVED_KEY_MAX_LEN];
	size_t derive_len;
	psa_status_t status;
	int result;
	...
	result = fp_fhn_eik_common_derive_key(eik, seed_end_byte, eik_derived_key,
					      derive_len);
	mbedtls_platform_zeroize(eik, sizeof(eik));
	if (result != 0) {
...
static psa_status_t tfm_eik_secure_dispatch(const struct fp_fhn_eik_secure_req *req,
					    const psa_msg_t *msg)
{
	switch (req->op) {
	case FP_FHN_EIK_SECURE_OP_DELETE:
		LOG_INF("[EIK SECURE] Delete");
		return tfm_eik_delete(msg);

	case FP_FHN_EIK_SECURE_OP_IS_PROVISIONED:
		LOG_INF("[EIK SECURE] Is provisioned");
		return tfm_eik_is_provisioned(msg);

	case FP_FHN_EIK_SECURE_OP_HASH_COMPARE:
		LOG_INF("[EIK SECURE] Hash compare");
		return tfm_eik_hash_compare(msg);

	case FP_FHN_EIK_SECURE_OP_EID_ENCODE:
		LOG_INF("[EIK SECURE] EID encode");
		return tfm_eik_eid_encode(msg);

	case FP_FHN_EIK_SECURE_OP_PROVISION_ENCRYPTED:
		LOG_INF("[EIK SECURE] Provision encrypted");
		return tfm_eik_provision_encrypted(msg);

	case FP_FHN_EIK_SECURE_OP_GET_ENCRYPTED:
		LOG_INF("[EIK SECURE] Get encrypted");
		return tfm_eik_get_encrypted(msg);

	case FP_FHN_EIK_SECURE_OP_DERIVE_KEY:
		LOG_INF("[EIK SECURE] Derive Key");
...
{
	struct fp_fhn_eik_secure_req req;
